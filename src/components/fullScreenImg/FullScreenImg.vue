<template>
    <div v-show="isShowFullImg" :class="['fullscreen', 'fullImg']">
      ...
      <div class="fullImgContent"  @touchstart="moveStart" @touchmove="moveTouch" @touchend="moveEnd" @click="test" ref='content'>
...
export default {
  ...
  methods: {
    ...
    moveTouch (e) {
      // 判斷左滑右滑
      let {direction: moveDir, movePoint} = this.swipeDirection(e.changedTouches[0].pageX)

      // 判斷對右側照片還是左側照片做移動
      let pageDir = this.pageDirection()
      if (pageDir === pageDirection.NONE && moveDir === moveDirection.Right) { // 上一頁
        // 對左側頁面右移
        this.pagePre(movePoint)
      } else if (pageDir === pageDirection.NONE && moveDir === moveDirection.Left) { // 下一頁
        // 對右側頁面左移
        this.pageNext(movePoint)
      } else if (pageDir === pageDirection.PrePage) { // 上一頁
        this.pagePre(movePoint)
      } else if (pageDir === pageDirection.NextPage) { // 下一頁
        this.pageNext(movePoint)
      }
    ...
    moveEnd () {
      ...
      // 左滑 中間右邊照片往左到指定位置，中間照片+1 右邊照片+1，瞬間回位
    },
    defaultPosition () {
    ...
    pagePre (movedPoint) {
      ...
      this.$refs.currentView.style.left = this.$refs.currentView.offsetLeft + moveP + 'px'
      this.$refs.preView.style.left = this.$refs.preView.offsetLeft + moveP + 'px'
    },
    pageNext (movedPoint) {
      this.$refs.currentView.style.left = this.$refs.currentView.offsetLeft + movedPoint + 'px'
      this.$refs.nextView.style.left = this.$refs.nextView.offsetLeft + movedPoint + 'px'
    }
